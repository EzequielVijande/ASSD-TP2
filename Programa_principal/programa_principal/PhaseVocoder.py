...
def PhVocoder(input,window,stretch_func,hop_size):
    ...
    for i in range(1,synth_vector.size):
        ...
        aux= np.fft.irfft(OutputSpectrum[i] )
        if(output_intervals[i].size > aux.size):
            (output_intervals[i][:aux.size]) =aux 
            output_intervals[i][aux.size:] = output_intervals[i][aux.size]
        else:
            output_intervals[i] = aux
