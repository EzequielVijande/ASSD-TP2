...
class Synthesizer:
    ...
    def synthesize(self, trk_info: TrackInfo, instrument: int, n_frames: int, first_time: bool):
        ...
        if len(self.x_out) < n_frames:
            for i in range(trk_info.get_total_notes()):
                ...
                if len(self.x_out) > n_frames:
                    self.curr_track_notes.remove(self.curr_track_notes[i])
                    self.refresh_notes(on_time, duration, instrument)
                    self.avg_counter = 0
    ...
    def refresh_notes(self, prev_on_time, prev_duration, instrument):
        back_up = self.curr_track_notes[:]
        self.off_all_notes(prev_on_time, prev_duration, instrument)
    ...
    def off_all_notes(self, prev_on_time, prev_duration, instrument):

        for on_ev, on_time, duration in self.curr_track_notes:
            if (on_time >= prev_on_time) and (on_time <= (prev_on_time+prev_duration)):
                pitch = on_ev.get_pitch()
                velocity = on_ev.get_velocity()
                beginning_n = int(on_time * self.frame_rate) - self.last_sent_n
                if (on_time+duration) >= (prev_on_time+prev_duration):
                    ending_n = int((prev_on_time+prev_duration) * self.frame_rate) + 1 - self.last_sent_n
                else:
                    ending_n = int((on_time+duration) * self.frame_rate) + 1 - self.last_sent_n
